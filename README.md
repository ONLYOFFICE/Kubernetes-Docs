...
| `upgrade.job.image.repository`                              | Job by upgrade image repository. The image must have `curl` and `psql` installed. The image must be run either from `root` or from `UID=101`                             | `postgres`                                   |
